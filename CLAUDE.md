...
`autonomy.py` levels: `scout` (scan only) → `paper` (paper execution) → `penny` (real $1-5) → `book` (Kelly-sized). Paper and penny persist isolated runtime state and scoped trade/accounting views, so paper experiments do not consume penny `max_open` capacity. Unattended launchd runs must use explicit scope selection via `AUTONOMY_BACKGROUND_SCOPES`; the default is paper-only, concurrent paper+penny loops are opt-in, and every cycle must log its `scope`/`runtime_label`. Penny parity with paper is the governing rule for strategy admission and execution: if a strategy/opportunity is admitted in paper, penny/book should attempt the same strategy under the same default admission logic unless a live-only safeguard vetoes that specific trade. Allowed penny vetoes are explicit execution safeguards such as verified-wallet/balance failure, slippage >2%, exchange/order rejection, unavailable market metadata, or other concrete live preflight/runtime failures that are logged with the blocking reason. Strategies must not introduce separate penny rollout toggles, stage buckets, scan-only defaults, or paper-only admission paths unless the operator explicitly asks for a temporary exception and the docs/fix logs are updated to say so. Exact-temperature weather execution remains the one approved exception and must stay explicitly labeled as such. Each cycle: scan pairs → scan weather → refresh open trades → auto-close reverted/resolved trades → open new trades up to the scoped `max_open`.
Manual dashboard-triggered autonomy runs (`POST /api/autonomy`) execute in a background thread, so runtime status/UI must clearly label that background execution and only report completion after all enabled phases finish. For penny/book scopes, every strategy phase must report whether it ran, opened trades, hit `slots_full`, or failed a live safeguard/error, with timing/count metadata and explicit veto reasons. Never describe an otherwise-eligible penny strategy phase as `scan-only` or `paper-only` unless the operator-approved exception is documented.
...
### Trading Modes
- **Paper** (default): simulates against current prices, tracks in SQLite. There are no limits on numbers of trades while in paper mode. Paper-runtime trades, balances, and autonomy state are isolated from the penny runtime, so open paper experiments do not block penny `max_open` limits or penny dashboard views.
...
- **Weather strategy parity**: threshold-weather parity is strategy-specific and exact. Penny/book weather must use the same horizon, disagreement, liquidity, edge, and reopen/dedupe admission checks as paper weather before execution is attempted. If a weather candidate passes in paper, penny may diverge only for a named live-only safeguard that is logged before execution and surfaced in runtime/UI; generic `live_safeguard_vetoed` labels without the concrete reason are not acceptable.
...
- **Live**: requires `POLYMARKET_PRIVATE_KEY` in the macOS Keychain (or an explicit per-process env override), and order submission needs Polymarket CLOB API credentials either supplied explicitly (`POLYMARKET_CLOB_API_KEY` / `POLYMARKET_CLOB_API_SECRET` / `POLYMARKET_CLOB_API_PASSPHRASE`) or derivable from that wallet key at runtime, uses py-clob-client
- **Parity contract**: penny/book are not narrower research lanes. Default strategy admission, AI/brain usage, weighting/sizing paths, and trade-attempt behavior should match paper. Cached Perplexity/Polygon/stage metadata may be logged and displayed, but must not create extra penny-only blockers unless the blocker is an explicit live safeguard on that trade.
- **Pre-execution parity**: do not mark penny weather as `tradeable`, `executing`, or otherwise ready-to-open if the same shared weather admission path will veto it moments later. The canonical admission/preflight decision must run before execution logging and before runtime/UI counts candidates as openable.
...
- **Penny slot accounting**: penny `max_open` capacity is one shared live budget across weather and cointegration, but it must be derived only from penny live-ledger positions. Paper rows, even if mislabeled with `runtime_scope="penny"`, must never consume penny slots. Operator-visible runtime status must show the active penny `max_open`, current usage, available slots, the specific open penny trades consuming capacity, and the explicit reason new penny trades are blocked when the live book is full.
...
# Check brain-provider runtime / cutover status
...
- `POLYMARKET_PRIVATE_KEY` — enables live trading and CLOB API-credential derivation
- `POLYMARKET_CLOB_API_KEY` / `POLYMARKET_CLOB_API_SECRET` / `POLYMARKET_CLOB_API_PASSPHRASE` — optional explicit Polymarket CLOB API credentials for order submission; set all three together or leave all three unset so the runtime derives them from `POLYMARKET_PRIVATE_KEY`
...
- `STAGE2_POLYGON_GATING` — `1`/`true`/`yes` turns on legacy Polygon audit instrumentation. It may add block/slippage metadata to logs and attempt records, but it must not be treated as a penny-only admission gate or a prerequisite for penny parity.

## Recent Fix Logs
- `fix_logs/2026-04-05-penny-parity-guidance-contract.md`: rewrote the repo guidance so penny parity with paper is the governing rule, reclassified staged live-gating docs as historical context only, and made explicit that only per-trade live safeguards may veto penny trades by default.
- `fix_logs/2026-04-05-penny-weather-phase-runtime-visibility.md`: manual dashboard autonomy runs now report their background execution explicitly; the historical paper-only weather-skip wording is archived and superseded by parity-first weather telemetry.
- `fix_logs/2026-04-05-penny-weather-live-rollout-scope.md`: historical note for the intermediate weather rollout lane; active guidance now requires penny weather to follow the same default admission path as paper, with only explicit live safeguard vetoes and the exact-temperature exception remaining.
- `fix_logs/2026-04-05-penny-live-slot-accounting.md`: centralized penny slot accounting on the live ledger only, kept weather/cointegration on one shared penny slot budget, and surfaced slot consumers plus available capacity in runtime logs and the dashboard.
- `fix_logs/2026-04-05-penny-slot-limit-ui-visibility.md`: promoted penny slot-limit state into a first-class runtime/dashboard contract so operators now see the active penny max-open setting, current open count, remaining slots, strategy-specific slot pressure, and the explicit reason new weather/cointegration trades are blocked when the live book is full.
- `fix_logs/2026-04-05-penny-weather-paper-parity.md`: removed the separate penny weather scan-only gate so threshold-weather follows the primary runtime control, and live safeguard vetoes now surface as explicit per-trade weather execution errors.
- `fix_logs/2026-04-05-penny-weather-strategy-parity-preflight.md`: moved threshold-weather horizon revalidation into the shared admission path, made weather tradeable counts reflect preflight-ready candidates instead of raw scan output, and required penny weather runtime/UI surfaces to expose the concrete veto reason and `blocker_source` before execution.
- `fix_logs/2026-04-05-autonomy-runtime-scheduler-isolation.md`: added explicit background-scope configuration for unattended autonomy, labeled every autonomy journal lane with its runtime scope, and kept paper-only background helpers out of penny unless they are redesigned and documented for parity-safe live use.
...
- `fix_logs/2026-04-03-stage2-polygon-gating.md`: legacy Stage 2 label retained for history; the flag now documents optional Polygon audit metadata only and must not be used as an extra penny gate.
...
- `fix_logs/2026-04-04-stage2-perplexity-validation.md`: legacy Stage 2 label retained for history; Perplexity verdicts are cached as signal metadata for observability and AI context, not as an extra penny-only promotion gate.
- `fix_logs/2026-04-04-stage3-perplexity-gating.md`: historical record of a removed gate; cached Perplexity verdicts may still be displayed and logged, but they no longer define a separate live bucket or penny admission requirement.
- `fix_logs/2026-04-04-grade-a-weighted-entries.md`: Grade-A trials now run smaller, grade-weighted entries (25–65% of the base book) with relaxed thresholds and explicit guardrail telemetry so the near-miss cohort can keep flowing through the logged stop/hold gates.
- `fix_logs/2026-04-05-penny-a-grade-parity.md`: removed the paper-only A-grade cointegration gate from penny/book, applied the weighted A-trial sizing path in live autonomy too, and now journal penny/book strategy-admission vetoes so parity gaps are explicit.

## Daily Report Updates
- 2026-04-03 report refreshed so the “Not Working” and “Top 5 Improvements” entries point to resolved fix logs, while its older Stage 2/3 framing is now archived and superseded by the parity-first contract.
- Status: system health is stable with the reopened issues closed. Polygon and Perplexity metadata remain available for observability, but penny behavior is no longer governed by Stage 2/3 promotion buckets or live-readiness gates beyond the explicit per-trade safeguards documented in `reports/2026-04-04-stage3-live-readiness.md`.
## Penny Safeguards
- Penny live exposure remains intentionally small via the `autonomy.py` `penny`/`book` levels. The canonical live safeguards are documented in `reports/2026-04-04-stage3-live-readiness.md`, but that checklist is parity-first: it exists to verify wallet/balance, slippage, quarter-Kelly sizing, key presence, and execution reporting, not to impose separate strategy-admission stages.
- Cached Perplexity verdicts, profitable-candidate tags, stage labels, and Polygon audit payloads are observability artifacts only unless a concrete live safeguard fails on that trade.
...
- Decide whether `math_engine.py:434-520` should keep `tradeable` strictly tied to every hard/soft filter; if Grade A near-misses are admitted through the trial path in paper, the same trial path must be available in penny/book by default unless a live safeguard vetoes the specific trade.
- Broaden the cointegration trial guardrails (`cointegration_trial.py:12-212`) by limiting the allowed fails to the soft `momentum_pass`/`spread_std_pass` filters, capping the misses at 1, and by slightly lowering `min_liquidity` while raising `max_slippage_pct` so A-grade near-A+ signals earn parity-trial trades without altering the grade label semantics (see `fix_logs/2026-04-04-grade-a-paper-promotion.md`).
...
- Weighted A-grade entries now respect the grade-weighted size (25–65% of the base size depending on the passed filters), log `grade_weight` plus `weighted_entry_size_usd`, and benefit from the relaxed min_z_abs/min_ev_pct/min_liquidity/max_slippage gate described in fix_logs/2026-04-04-grade-a-weighted-entries.md so the existing A-grade signals start streaming raw experience data.
