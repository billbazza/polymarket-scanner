...
- **Penny slot accounting**: penny `max_open` capacity is one shared live budget across weather and cointegration, but it must be derived only from penny live-ledger positions. Paper rows, even if mislabeled with `runtime_scope="penny"`, must never consume penny slots. Operator-visible runtime status must show current usage, available slots, and the specific open penny trades consuming capacity.
