...
- **Dashboard/runtime semantics**: `/api/runtime/account` is the canonical dashboard account endpoint. Paper scope reports bankroll accounting; penny scope reports verified Polygon wallet-backed cash plus only penny-scoped positions/PnL. Penny mode is fail-closed: if live Polygon wallet data is unavailable, stale, or cannot be verified, the backend must error/block rather than falling back to paper balances, paper PnL, or shared aggregate totals, and the UI must show an explicit `LIVE / POLYGON WALLET` status.
