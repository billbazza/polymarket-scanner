...
- `fix_logs/2026-04-04-stage3-perplexity-gating.md`: Stage 3 readiness now trusts the cached Perplexity verdict (`profitable_candidate_feature`) so only those profitable candidates get promoted to the live bucket and the dashboard can highlight the stage 3-ready set.
...
- `fix_logs/2026-04-05-penny-a-grade-parity.md`: removed the paper-only A-grade cointegration gate from penny/book, applied the weighted A-trial sizing path in live autonomy too, and now journal penny/book strategy-admission vetoes so live parity gaps are explicit.
...
- Stage 3 readiness now depends on the cached Perplexity verdict so only profitable candidates flow into the live bucket, and the dashboard filter/indicator plus journal/audit logging are captured in `fix_logs/2026-04-04-stage3-perplexity-gating.md`.
...
- Decide whether `math_engine.py:434-520` should keep `tradeable` strictly tied to every hard/soft filter; grade "A" soft misses (e.g., `momentum_pass`, `spread_std_pass`) stay blocked at stage 1 so the cointegration A-trial can safely log the failures and apply secondary guardrails without breaking penny parity.
