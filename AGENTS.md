...
- Decide whether `math_engine.py:434-520` should keep `tradeable` strictly tied to every hard/soft filter or if grade "A" candidates that only fail soft checks (e.g., `momentum_pass`, `spread_std_pass`) can flip `tradeable=True` via a config flag so they appear in stage-1 logging without changing the grade label semantics.
- Broaden the cointegration trial guardrails (`cointegration_trial.py:12-212`) by allowing a second failed filter, permitting frequent misses such as Kelly/momentum/spread_std in `allowed_failed_filters`, or tuning `min_z_abs`, `min_ev_pct`, `min_liquidity`, and slippage thresholds so the thousands of grade "A" signals get a clear path instead of being categorized as `filter_failure_outside_trial`.
