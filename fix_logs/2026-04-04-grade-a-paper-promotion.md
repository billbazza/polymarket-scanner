...
- The latest autonomy run requested more detail about those fails so operators could tell whether a Grade A was a 7/8 near-miss or a lower-quality rejection before touching live money.
...
- Loosened the guardrails slightly (lower `min_liquidity`, higher `max_slippage_pct`, and a marginally softer `min_ev_pct`) so these near-A+ signals still publish workable guardrails without diluting the stage-1 tradeable cohort.
- Continued to keep `tradeable=True` tied strictly to the A+ math outcome while logging the guardrail tweaks and failed-filter counts for every A-grade attempt.
