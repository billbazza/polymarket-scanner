...
- Followed the Architecture, Rules, and Always Do sections of [AGENTS.md](/Users/will/.cline/worktrees/eb200/polymarket-scanner/AGENTS.md).

## Preconditions Reviewed
- Paper trade admission is now centralized and visible through structured blockers for pairs and weather in [db.py](/Users/will/.cline/worktrees/eb200/polymarket-scanner/db.py).
- Paper trade state is explicitly split from wallet-attached and live exchange states, and paper utilization excludes non-paper states in [db.py](/Users/will/.cline/worktrees/eb200/polymarket-scanner/db.py).
- Paper accounting and strategy bankroll utilization are derived from consistent mark-to-market logic in [db.py](/Users/will/.cline/worktrees/eb200/polymarket-scanner/db.py) and surfaced via `/api/paper-account`.
...
- Even if `rollout_state=active` and `active_policy=confidence_aware` are set, the framework now rolls back to `fixed` unless all three machine-checked requirements pass in [paper_sizing.py](/Users/will/.cline/worktrees/eb200/polymarket-scanner/paper_sizing.py):
...
- Produce a new dated review note from fresh paper samples before changing the readiness acknowledgements or relying on confidence-aware paper execution more broadly.
