...
4. **Runtime keys and market preflight** – `POLYMARKET_PRIVATE_KEY` and `ALCHEMY_API_KEY` must be present in the macOS Keychain service used by `runtime_config.py` (or be injected as explicit per-process env overrides), and the live execution path must be able to fetch the market/order prerequisites required for the specific trade. If not, the trade is vetoed and logged.
